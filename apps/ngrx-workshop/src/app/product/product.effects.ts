import { Injectable } from '@angular/core';
import { Actions, concatLatestFrom, createEffect, ofType } from '@ngrx/effects';
import { of } from 'rxjs';
import { catchError, exhaustMap, filter, map, switchMap } from 'rxjs/operators';

import { productApiActions } from './actions';
import * as productListActions from './product-list/actions';
import { ProductService } from './product.service';
import { productDetailsActions } from "./product-details/actions";
import { Store } from "@ngrx/store";
import { selectCurrentProductId } from "./product.selectors";

@Injectable()
export class ProductEffects {
  constructor(
    private readonly actions$: Actions,
    private readonly productService: ProductService,
    private readonly store: Store,
  ) {}

  fetchProducts$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(productListActions.productsOpened),
      exhaustMap(() =>
        this.productService.getProducts().pipe(
          map((products) =>
            productApiActions.productsFetchedSuccess({ products })
          ),
          catchError(() =>
            of(
              productApiActions.productsFetchedError({
                errorMessage: 'Error Fetching Products',
              })
            )
          )
        )
      )
    );
  });

  fetchCurrentProduct$ = createEffect(() => { // If you see an error here, probably a signature issue by not returning an Action yet
    return this.actions$.pipe(
      ofType(productDetailsActions.pageOpened),
      concatLatestFrom(() =>
        this.store.select(selectCurrentProductId).pipe(
          // TS won't know it's not null/undefined later. Can also do `is NonNullable<typeof id>`
          // Don't use `filter<>` to force a type bc unreliable
          filter((id): id is string => id != null)
        )
      ),
      switchMap(([, id]) =>
        this.productService.getProduct(id).pipe(
          map((product) =>
            productApiActions.singleProductFetchedSuccess({ product })
          ),
          catchError(() =>
            of(
              productApiActions.singleProductFetchedError({
                errorMessage: 'Error Fetching Single Product',
              })
            )
          )
        )
      )
    );
  });
}
